...
const { Accessory, Service, Characteristic, AccessoryEventTypes, uuid, AdaptiveLightingController } = require('../modules/hap-nodejs');
        ...
        for (const prefix of capabilities) {
          ...
          if (! service) {
            ...
            if (typeof map.onService === 'function') {
              ...
              // Attach Adaptive Lighting if this is a tunable white light
if (service instanceof Service.Lightbulb && service.testCharacteristic(Characteristic.ColorTemperature)) {
  try {
    const alController = new AdaptiveLightingController(service);
    accessory.configureController(alController);
    this.log(`Adaptive Lighting enabled for ${device.name}`);
  } catch (err) {
    this.log(`Adaptive Lighting not available for ${device.name}: ${err.message}`);
  }
}
            }
          }
