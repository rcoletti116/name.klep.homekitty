...
if (! service) {
  ...
  // 👇 ADD THIS
  if (
    map.adaptiveLighting &&
    service.testCharacteristic(Characteristic.ColorTemperature)
  ) {
    const { AdaptiveLightingController } = require('hap-nodejs');
    this.log(4, '- enabling Adaptive Lighting Controller');
    try {
      const alc = new AdaptiveLightingController(service);
      service.configureController(alc);
    } catch (err) {
